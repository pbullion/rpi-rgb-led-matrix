            ...
            for item in responseArr:
                ...
                if type(item) is dict and 'league' in item.keys() and item['league'] == 'mlb':
                    ...
                    if item['pregame'] == True:
                        ...
                        for offset in range(13):
                            graphics.DrawLine(canvas, 0, offset + 13, oddsStartNum - 3, offset + 18, homeColorPrimary)
                        awayTeam = graphics.DrawText(canvas, smallFont, 1, 11, awayColorSecondary, item['awayTeam']['teamName'])
                        awayPitcher = graphics.DrawText(canvas, smallFont, 1, 11, awayColorSecondary, 'J. Verlander 3-1 1.50 ERA')
                        homeTeam = graphics.DrawText(canvas, smallFont, 1, 24, homeColorSecondary, item['homeTeam']['teamName'])
                        homePitcher = graphics.DrawText(canvas, smallFont, 1, 28, awayColorSecondary, 'J. Verlander 3-1 1.50 ERA')
