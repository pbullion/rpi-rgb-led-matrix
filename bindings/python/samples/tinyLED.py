        ...
        while True:
            ...
            for item in responseArr:
                ...
                if type(item) is dict and 'league' in item.keys() and item['league'] == 'mlb':
                    ...
                    print(item)
                    tournamentName = graphics.DrawText(canvas, slightlyBiggerFont, 0, 5, green, item['tourneyName'])
                    tourneyStatus = graphics.DrawText(canvas, smallestFont, 0, 15, blue, item['status'])
                    running = True
                    while running:
                        pos -= 1
                        topGolfers = graphics.DrawText(canvas, middleFont, pos, 27, purple, item['topGolfers'])
                        if (pos + topGolfers < 0):
                            running = False
                            pos = canvas.width
                        time.sleep(0.008)
